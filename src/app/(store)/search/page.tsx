...
const SearchPage = async ({searchParams}:{searchParams:Promise<{query:string}>}) => {
    ...
    const products = (await searchProductByName(query)).map(product => ({
      ...product,
      // slug: product.slug || ''
    }));
    if (!products.length){
      return(
        <div className="flex flex-col items-center justify-top min-h-screen bg-gray-100 p-4">
          <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-4xl">
            <h1 className="text-3xl font-bold mb-6 text-center"> No prdoucts found for: {query}</h1>
  ...
  return (
    <div className="flex flex-col items-center justify-top min-h-screen bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-4xl">
        <h1 className="text-3xl font-bold mb-6 text-center">Search page for {query}</h1>
